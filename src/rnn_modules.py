...
class PeepholedLSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, bias=False):
        ...
        #####################################################################
        # Learnable weights and bias for `foreget gate`
        ...
        # Learnable weights and bias for `new memory cell`
        self.W_c = nn.Parameter(torch.Tensor(hidden_size, hidden_size + input_size))
    ...
    def forward(self, x, prev_state):
        ...
        prev_concat_hxc = torch.cat((prev_c, prev_h, x), dim=1)
        f = torch.sigmoid(F.linear(prev_concat_hxc, self.W_f, self.b_f))
        i = torch.sigmoid(F.linear(prev_concat_hxc, self.W_i, self.b_i))

        concat_hx = torch.cat((prev_h, x), dim=1)
        c_tilde = torch.tanh(F.linear(concat_hx, self.W_c, self.b_c))

        next_c = f * prev_c + i * c_tilde
        next_concat_hxc = torch.cat((next_c, prev_h, x), dim=1)
        o = torch.sigmoid(F.linear(next_concat_hxc, self.W_o, self.b_o))
